...
WORKDIR /app
RUN latest=$(curl -sSL https://maven.neoforged.net/releases/net/neoforged/neoforge/maven-metadata.xml | grep "latest" | sed -n 's/.*<version>\(.*\)<\/version>.*/\1/p') && curl -4 -L --http1.1 "https://maven.neoforged.net/releases/net/neoforged/neoforge/$latest/neoforge-$latest-installer.jar" -o installer.jar
